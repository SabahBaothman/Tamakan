...
slide_summaries = [
    ...
    "Summary of Slide 14: Convolutional Neural Networks (CNN) are the most popular deep learning architecture in computer vision. The most important feature to classify images is the shape. To extract the shape features we need to extract the edges.",
    "Summary of Slide 15: Convolutions have been used for a long time in image processing to blur and sharpen images. A convolution operation is an element-wise matrix multiplication operation. The output of this is the final convoluted image.",
    ...
    "Summary of Slide 32: A CNN model can be thought of as a combination of two components: feature extraction and classification. The convolution and pooling layers perform feature extraction. The fully connected layers act as a classifier on top of these features.",
    "Summary of Slide 33: Convolutional Networks are made up of only three types of layers: CONV, POOL (we assume Max pool unless stated otherwise), and FC (short for fully-connected).",
...
# Function to evaluate student's explanation using OpenAI GPT API
